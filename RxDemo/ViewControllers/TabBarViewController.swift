...
class TabBarViewController: UITabBarController {
    ...
    func configViewController() {
        let childClassNames = [
            ...
            ["ClassName": "MessageViewController", "Name": "消息"],
            ["ClassName": "MineZoneViewController", "Name": "我"]
        ]
    ...
    func buildViewController(from conf:Dictionary<String, String>) -> UIViewController {
        guard let className = conf["ClassName"],
            ...
            temp.view.backgroundColor = UIColor(red: 59, green: 89, blue: 152 ,alpha: 1.0)
            return temp
        }
    ...
    func buildTabbarItem(from conf:Dictionary<String, String>) -> CustomTabbarItem {
        guard let name = conf["Name"] else {
            guard let image = conf["Image"] else {
                return TitleTabbarItem(title: "无")
            }
...
class TitleTabbarItem: UIView, CustomTabbarItem {
    ...
    func setUpUI() {
        ...
        indicatorView.bottomAnchor.constraint(equalTo: bottomAnchor, constant: -8).isActive = true
...
class CustomTabBar: UITabBar {
    ...
    private func layoutTabbarItem() {
        ...
        let itemW = frame.size.width / CGFloat(items.count)
        for (index, item) in items.enumerated() {
            ...
            if item is TitleTabbarItem {
                item.frame = CGRect(x: itemW*CGFloat(index), y: 0, width: itemW, height: frame.size.height)
